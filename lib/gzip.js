  ...
  return function gzip(req, res, next) {
    ...
    res.writeHead = function (code) {
      var args = Array.prototype.slice.call(arguments, 0),
        ...
        headers, key, accept, type, encoding, gzipProcess, ua;
      if (args.length > 1) {
        headers = args.pop();
        for (key in headers) {
      ...
      if (req.method === 'HEAD' || code !== 200 || accept.indexOf('gzip') === -1 || !matchType.test(type) || encoding ||
      ...
      gzipProcess = spawn(bin, flags);

      res.write = function (chunk, encoding) {
        gzipProcess.stdin.write(chunk, encoding);
      };
      ...
      res.end = function (chunk, encoding) {
        ...
        gzipProcess.stdin.end();
      };

      gzipProcess.stdout.addListener('data', function (chunk) {
        write.call(res, chunk);
      });

      gzipProcess.addListener(version[0] === 0 && version[1] < 7 ? "exit" : "close", function (code) {
        res.write = write;
        res.end = end;
        res.end();
      });
